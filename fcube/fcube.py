...
def sign_projection(M, new_dim, seed=None):
    """
    ...
    if small:
        ...
        M_red = scipy.sparse.csr_matrix.dot(M,
                                            np.reshape(S, [old_dim, new_dim]))
    else:
...
def gauss_projection(M, new_dim, G=None, seed=None):
    """
    Project the columns of the matrix M into the lower dimension new_dim using
    gaussian
    """
    np.random.seed(seed=seed)
    m, old_dim = np.shape(M)
    ...
    if small:
        if G is not None:
            ...
            M_red = scipy.sparse.csr_matrix.dot(M, np.random.randn(old_dim,
                                                                   new_dim))
    else:
        for i in range(new_dim):
...
def countSketch_projection(M, new_dim, seed=None):
    """
    Project the columns of the matrix M into the lower dimension new_dim using
    count sketch
    """
...
def countGauss_projection(M, new_dim, seed=None):
    """
    Project the columns of the matrix M into the lower dimension new_dim using
    count sketch + gaussian algorithm
    """
    np.random.seed(seed)
    m, old_dim = np.shape(M)

    # ksq = new_dim * new_dim
    ksq = np.int(new_dim * 5)  # was not converging for scree plots of damle/sun
